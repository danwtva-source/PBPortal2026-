...
import { COMMITTEE_DOCS, SCORING_CRITERIA } from '../constants';
    ...
    return (
        <div className="max-w-3xl mx-auto py-8 px-4 animate-fade-in">
            ...
            <Card className="border-t-4 border-t-brand-purple">
                <div className="mb-6">
                    <h2 className="text-2xl font-bold font-dynapuff text-gray-800">Expression of Interest (Part 1)</h2>
                    <p className="text-gray-500 text-sm">Round 2 • 2026 Cycle</p>
                </div>
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
            ...
            <Button onClick={() => setIsCreating(true)} className="shadow-xl">+ Start New Application</Button>
        </div>
                    ...
                    <div className="flex gap-6 items-center w-full">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <Badge variant={app.status === 'Submitted' ? 'blue' : 'green'}>{app.status}</Badge>
                            </div>
...
export const CommitteeDashboard: React.FC<CommitteeProps> = ({ user, onUpdateUser, isAdmin, onReturnToAdmin }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Get all scores to calculate progress
            const allScores = await api.getScores();
            // Filter for just MY scores
            const userScores = allScores.filter(s => s.scorerId === user.uid);
    ...
    const handleSaveScore = async (isFinal = false) => {
        ...
        const allScores = await api.getScores();
        setMyScores(allScores.filter(s => s.scorerId === user.uid));
        ...
        return (
            <div className="h-[calc(100vh-80px)] flex flex-col md:flex-row animate-fade-in">
                ...
                <div className="md:w-2/3 bg-gray-100 flex flex-col">
                    {activeApp ? (
                        <iframe src={activeApp.pdfUrl} className="w-full h-full border-0" title="PDF" />
                    ) : (
...
export const AdminDashboard: React.FC<AdminProps> = ({ onNavigatePublic, onNavigateScoring }) => {
    ...
    const handleCreateUser = async (e: React.FormEvent) => {
        ...
        // For this demo, let's just close it to show the modal flow
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y">
                                {allApps.map(app => {
                                    ...
                                    const avg = appScores.length ? Math.round(appScores.reduce((a,b) => a + b.total, 0) / appScores.length) : 0;
                                    return (
                                        <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="p-5"><Badge variant={app.status === 'Submitted' ? 'blue' : 'gray'}>{app.status}</Badge></td>
